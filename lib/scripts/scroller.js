export default function createScroller(scrollConfig) {
  ...
  function updateProgress() {
    if (scrollConfig.progress) {
      const sectionsTotal = $(".jpage .section").length;
      const progressBar = $('.jpage-scroll-progress')
      if (open > 0) {
        progressBar.css("width", `calc(100vw * (${(open + 1) / sectionsTotal}))`)
      } else {
  ...
  if (scrollConfig.progress) {
    const progressBar = $('<div class="jpage-scroll-progress"></div>')
    progressBar.css("background", scrollConfig.progressColor)
    jpage.append(progressBar)
  }
}
